...
output = numpy.zeros((repeats, samples, 2))
labels = []

for repeat in range(repeats):
    pole_present = random.choice([0, 1])

    left_echo = numpy.zeros(samples)
    right_echo = numpy.zeros(samples)
    pole_echo = numpy.zeros(samples)
    pole_index = random.choice(range(n_poles))
    pole_echo[start_pole - delta_pole:end_pole - delta_pole] = poles[pole_index, :]
    pole_echo = pole_echo * pole_present

    for az_index in range(len(azs)):
        left_gain = left_gains[az_index]
        right_gain = right_gains[az_index]
    ...
    output[repeat,:,0] = left_wiegrebe_total
    output[repeat, :, 1] = right_wiegrebe_total
    labels.append(pole_present)
